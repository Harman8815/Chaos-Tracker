...
const TrajectoryChart = ({ dailyTotals, daysInMonth }: { dailyTotals: { [day: number]: number }, daysInMonth: number }) => {
    ...
    if (dataPoints.every(p => p === 0)) return <div className="h-16 border-b-2 border-border" style={{ width: `${width}px` }}></div>;
...
const PointsTracker: React.FC = () => {
    ...
    const { setIsEditHabitsModalOpen, setIsEditRulesModalOpen, scoringRules } = useContext(SettingsContext);
    ...
    const handleScoreClick = (date: string, habitId: string) => {
        ...
        setData(prev => {
            ...
            return {
                ...prev,
                [date]: { ...dayData, habitScores: updatedHabitScores, points: newTotalPoints },
            };
        });
    };
