...
class NotificationService:
    ...
    def update_preferences(self, user, data: dict) -> NotificationPreference:
        ...
        if "quiet_hours_start" in data:
            prefs.quiet_hours_start = (
                self._parse_time(data["quiet_hours_start"]) if data["quiet_hours_start"] else None
            )
        if "quiet_hours_end" in data:
            prefs.quiet_hours_end = (
                self._parse_time(data["quiet_hours_end"]) if data["quiet_hours_end"] else None
            )
        if "timezone" in data:
    ...
    def create_notification(
        ...
        if dedupe_key:
            ...
            if exists:
                logger.debug(
                    "notifications.deduplicated user_id=%s type=%s key=%s",
                    user.id,
                    notification_type,
                    dedupe_key,
                )
...
class ScheduledJobService:
    ...
    def create_job(
        ...
        logger.info(
            "jobs.created type=%s scheduled_at=%s user_id=%s",
            job_type,
            scheduled_at,
