...
class AnalyticsService:
    ...
    def get_consistency_score(self, user, *, days=30):
        ...
        for agg in aggregates:
            total_possible += agg.habits_total + (1 if agg.habits_total > 0 else 0)  # habits + journal
    ...
    def generate_insights(self, user, *, days=30):
        ...
        if consistency["score"] >= 80:
            ...
            insights.append({
                ...
                "message": f"Only {consistency['score']}% completion rate. Consider reducing habit targets or focusing on 1-2 key habits.",
                "domain": "consistency",
            })
        ...
        if velocity["goals_completed"] > 0:
            if velocity["velocity_per_week"] >= 1:
                insights.append({
                    ...
                    "message": f"Completing {velocity['velocity_per_week']} goals/week. Average completion time: {velocity['avg_completion_days']} days.",
                    "domain": "goals",
                })
            else:
                insights.append({
                    ...
                    "message": f"Only {velocity['goals_completed']} goals completed in {days} days. Consider breaking goals into smaller milestones.",
                    "domain": "goals",
                })
        else:
            insights.append({
        ...
        if financial["savings_rate"] >= 20:
            ...
            insights.append({
                ...
                "message": f"Spending ${abs(financial['net_flow']):.2f} more than income. Review top expense categories: {', '.join(list(financial['category_breakdown'].keys())[:3])}.",
                "domain": "finance",
            })
        ...
        for key, data in correlations.get("correlations", {}).items():
            if data["strength"] == "strong" and data["direction"] == "positive":
                ...
                insights.append({
                    "type": "insight",
                    "title": f"{d1.replace('_', ' ').title()} Correlates with {d2.replace('_', ' ').title()}",
                    "message": f"When {d1.replace('_', ' ')} increases, {d2.replace('_', ' ')} tends to increase too (r={data['coefficient']}).",
                    "domain": "correlation",
                })
    ...
    def get_productivity_score(self, user, *, days=30):
        ...
        water_days = sum(1 for a in aggregates if a.water_glasses >= a.water_target and a.water_target > 0)
        ...
        goals_created = Goal.objects.filter(user=user, created_at__date__range=[start_date, end_date]).count()
