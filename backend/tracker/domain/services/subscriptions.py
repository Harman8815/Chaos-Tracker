...
class SubscriptionService:
    ...
    def create(self, user, data):
        ...
        billing_cycle = validation.bounded_text(data.get("billing_cycle"), max_length=20, field="billing_cycle")
        if billing_cycle not in dict(Subscription.BILLING_CYCLES):
            raise ValidationError(f"Invalid billing_cycle. Must be one of: {[k for k, _ in Subscription.BILLING_CYCLES]}")
        next_billing_date = validation.parse_date(data.get("next_billing_date"), field="next_billing_date")
        ...
        status = validation.bounded_text(data.get("status", "active"), max_length=20, field="status")
        if status not in dict(Subscription.STATUS_CHOICES):
            raise ValidationError(f"Invalid status. Must be one of: {[k for k, _ in Subscription.STATUS_CHOICES]}")
    ...
    def update(self, user, subscription_id, data, *, partial=True):
        ...
        if "category" in data:
            subscription.category = validation.bounded_text(data["category"], max_length=100, field="category")
        if "amount" in data:
        ...
        if "billing_cycle" in data:
            billing_cycle = validation.bounded_text(data["billing_cycle"], max_length=20, field="billing_cycle")
            if billing_cycle not in dict(Subscription.BILLING_CYCLES):
                raise ValidationError(f"Invalid billing_cycle. Must be one of: {[k for k, _ in Subscription.BILLING_CYCLES]}")
            subscription.billing_cycle = billing_cycle
        if "next_billing_date" in data:
            subscription.next_billing_date = validation.parse_date(data["next_billing_date"], field="next_billing_date")
        if "start_date" in data:
            subscription.start_date = validation.parse_date(data["start_date"], field="start_date")
        if "end_date" in data:
            subscription.end_date = validation.parse_date(data["end_date"], field="end_date") if data["end_date"] else None
        if "status" in data:
    ...
    def summary(self, user):
        ...
        category_breakdown = {}
        for sub in active:
            category_breakdown[sub.category] = category_breakdown.get(sub.category, 0.0) + float(sub.amount)
        return {
