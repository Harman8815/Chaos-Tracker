...
class ContextBuilder:
    ...
    def _get_habits_summary(self) -> List[Dict]:
        ...
        for h in habits:
            today_score = DailyHabitScore.objects.filter(
                user=self.user, habit=h, date=date.today()
            ).first()
            result.append(
                {
                    "id": h.id,
    ...
    def _get_recent_journal(self, days: int = 7) -> List[Dict]:
        since = date.today() - timedelta(days=days)
        entries = JournalEntry.objects.filter(user=self.user, date__gte=since).order_by("-date")[
            :10
        ]
        return [
            {
        ...
        for b in budgets:
            spent = (
                Expense.objects.filter(
                    ...
                    total=Sum("quantity", output_field=None) * Sum("price", output_field=None)
                )[
                    "total"
                ]
                or 0
    ...
    def _get_conversation_history(self, limit: int) -> List[Dict]:
        if not self.conversation:
            return []
        messages = AIMessage.objects.filter(conversation=self.conversation).order_by("-created_at")[
            :limit
        ]
        return list(
            reversed(
