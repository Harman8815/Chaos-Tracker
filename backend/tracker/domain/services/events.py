...
class EventService:
    ...
    def record(
        self, user, event_type, *, subject_type="", subject_id="", payload=None, occurred_at=None
    ):
        ...
        logger.info(
            "events.record user_id=%s type=%s subject=%s/%s",
            user.id,
            event_type,
            subject_type,
            subject_id,
    ...
    def count_by_type(self, user, *, start_date=None, end_date=None):
        ...
        rows = qs.values("event_type").order_by().annotate(count=models.Count("id"))
