...
class IncomeService:
    ...
    def list_with_summary(self, user, *, year=None, month=None, source=None, start_date=None, end_date=None):
        incomes = self.list(
            user,
            year=year,
            month=month,
        ...
        for income in incomes:
            source_breakdown[income.source] = source_breakdown.get(income.source, 0.0) + float(income.amount)
        return {
            "count": len(incomes),
