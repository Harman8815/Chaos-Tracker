...
class HabitService:
    ...
    def create(self, user, data):
        ...
        schedule_days = validation.bounded_list(data.get("schedule_days", []), max_length=7, field="schedule_days")
        schedule_days = sorted({_normalize_weekday(d) for d in schedule_days})
        reminders = validation.bounded_list(data.get("reminders", []), max_length=20, field="reminders")
        grace_period = validation.in_range(data.get("grace_period", 0), 0, 365, field="grace_period")
    ...
    def update(self, user, habit_id, data, *, partial=True):
        ...
        if "schedule" in data:
            habit.schedule = validation.choice(data["schedule"], Habit.SCHEDULE_KEYS, field="schedule")
        if "schedule_days" in data:
            days = validation.bounded_list(data["schedule_days"], max_length=7, field="schedule_days")
            habit.schedule_days = sorted({_normalize_weekday(d) for d in days})
        if "reminders" in data:
            habit.reminders = validation.bounded_list(data["reminders"], max_length=20, field="reminders")
        if "grace_period" in data:
            habit.grace_period = validation.in_range(data["grace_period"], 0, 365, field="grace_period")
        if "goal" in data:
