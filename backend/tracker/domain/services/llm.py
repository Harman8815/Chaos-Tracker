...
- Don't make up data - use tools to get real information"""
    ...
    def _format_context(self, context: Dict) -> str:
        ...
        if goals:
            ...
            for g in goals[:5]:
                lines.append(
                    f"  - {g.get('text', 'N/A')} [{g.get('category', 'N/A')}] "
    ...
    def _build_messages(
        self, context: Dict, conversation_history: List[Dict], user_message: str
    ) -> List[Dict]:
    ...
    def _fallback_response(self, context: Dict, user_message: str) -> LLMResponse:
        ...
        content = responses.get(
            intent, f"I understand you want to {
                intent.replace(
                    '_',
