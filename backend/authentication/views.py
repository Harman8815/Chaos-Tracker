    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return success_response(
                data={"message": "Account created successfully", "user": UserSerializer(user).data},
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user is not None:
                ...
                return success_response(
                    data={"message": "Login successful", "user": UserSerializer(user).data},
...
class LogoutView(views.APIView):
    ...
    def post(self, request):
        logout(request)
        return success_response(
            data={"message": "Logged out successfully"}, status_code=status.HTTP_200_OK
        )
...
class CurrentUserView(views.APIView):
    ...
    def get(self, request):
        return success_response(
            data={"user": UserSerializer(request.user).data}, status_code=status.HTTP_200_OK
        )
