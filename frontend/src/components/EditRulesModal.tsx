                        ...
                        <div key={rule.id} className="p-3 bg-white/[0.06] rounded-lg space-y-2">
                             <div className="grid grid-cols-12 gap-4 items-center">
                                <div className="col-span-3">
                                    ...
                                    <Input type="text" value={rule.activity} onChange={e => handleRuleChange(rule.id, 'activity', e.target.value)} />
                                </div>
                                <div className="col-span-2">
                                    <label className="text-xs text-text-secondary">Max Points</label>
                                    <Input type="number" value={rule.maxPoints} onChange={e => handleRuleChange(rule.id, 'maxPoints', parseInt(e.target.value))} />
                                </div>
                                <div className="col-span-6">
                                     <label className="text-xs text-text-secondary">Penalty Rule</label>
                                    <Input type="text" value={rule.penaltyRule} onChange={e => handleRuleChange(rule.id, 'penaltyRule', e.target.value)} />
                                </div>
                               ...
                               <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input type="text" value={rule.zeroPointsCondition} onChange={e => handleRuleChange(rule.id, 'zeroPointsCondition', e.target.value)} />
                                </div>
                                <div>
                                    <label className="text-xs text-text-secondary">Scoring Logic / Notes</label>
                                    <Input type="text" value={rule.scoringLogic} onChange={e => handleRuleChange(rule.id, 'scoringLogic', e.target.value)} />
                                </div>
                              </div>
                        </div>
                    ))}
