...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        if (mounted && distRes?.habits)
          setTodayDistributionServer(distRes.habits);
  ...
  const todayPieData = useMemo(() => {
    if (
      todayDistributionServer &&
      Array.isArray(todayDistributionServer) &&
      todayDistributionServer.length
    ) {
      return todayDistributionServer.map((h: any) => ({
        name: h.name,
        value: h.score,
      }));
    }
  ...
  const radarData = useMemo(() => {
    ...
    return habits.map((habit) => {
      const totalScore = last7Days.reduce(
        (sum, [, dayData]) =>
          sum + ((dayData as DailyData).habitScores?.[habit.id] || 0),
        0,
      );
      return {
        axis: habit.name,
  ...
  const overallProgressData = useMemo(() => {
    ...
    const totalAchieved = Object.entries(data)
      .filter(
        ([date]) =>
          new Date(date) >= monthStart && new Date(date) <= currentDate,
      )
      .reduce((total, [, dayData]) => {
        return (
          total +
          Object.values((dayData as DailyData).habitScores || {}).reduce(
            (sum, score) => sum + (Number(score) || 0),
            0,
          )
        );
    ...
    const totalHabitTarget = habits.reduce(
      (acc, habit) => acc + habit.target,
      0,
    );
    ...
    return [
      ...
      {
        name: "Remaining",
        value: Math.max(0, totalHabitTarget - totalAchieved),
      },
    ];
  }, [data, habits, today]);
  ...
  const weeklyPerformance = useMemo(() => {
    ...
    Object.entries(data).forEach(([date, val]) => {
      ...
      const dailyTotal = Object.values(
        (val as DailyData).habitScores || {},
  ...
  const levelStats = useMemo(() => {
    ...
    Object.values(data).forEach((day) => {
      ...
      if (d.habitScores) {
        totalXP += Object.values(d.habitScores).reduce(
