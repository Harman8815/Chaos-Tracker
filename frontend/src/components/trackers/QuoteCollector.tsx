        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 animate-fade-in" onClick={onClose}>
            <div className="bg-[rgba(15,10,30,0.75)] p-8 rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <input placeholder="Title" value={title} onChange={e => setTitle(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" required />
                    <select value={type} onChange={e => setType(e.target.value as QuoteSource['type'])} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]">
                        <option>Movie</option>
                        <option>Web Series</option>
                        <option>Book</option>
                    </select>
                    <input placeholder="Cover Image URL" value={coverImage} onChange={e => setCoverImage(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" required />
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 animate-fade-in" onClick={onClose}>
            <div className="bg-[rgba(15,10,30,0.75)] p-8 rounded-xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <textarea placeholder="Quote text..." value={text} onChange={e => setText(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)] h-24" required />
                    <input placeholder="Author" value={author} onChange={e => setAuthor(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" required />
                    <input placeholder="Tags (comma-separated)" value={tags} onChange={e => setTags(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" />
                    <input placeholder="Optional Image URL" value={image} onChange={e => setImage(e.target.value)} className="w-full p-2 rounded-md bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" />
                    <div className="flex justify-end gap-4 pt-4">
                        <Button type="button" onClick={onClose} className="bg-[rgba(15,10,30,0.6)] text-white hover:bg-[rgba(139,92,246,0.35)]">Cancel</Button>
            ...
            <div className="p-6 h-full overflow-y-auto animate-fade-in relative">
                ...
                <div className="flex flex-col md:flex-row items-start gap-6 mb-8">
                    ...
                    <div className="flex-grow">
                        ...
                        <p className="text-[#e9d5ff] mb-4">{selectedSource.type}</p>
                    ...
                    {Object.entries(groupedQuotes).map(([tag, quotes]) => (
                        <div key={tag}>
                            ...
                            <div className="space-y-4">
                                {quotes.map(quote => (
                                    <blockquote key={quote.id} className="p-4 border-l-4 border-[rgba(139,92,246,0.35)] bg-[rgba(15,10,30,0.75)] rounded-r-lg shadow-sm group relative">
                                        ...
                                        <cite className="block text-right mt-2 text-[#e9d5ff] not-italic">&mdash; {quote.author}</cite>
                                    </blockquote>
                                ))}
                            </div>
                        </div>
    ...
    if (activeSearch) {
        return (
            <div className="p-6 h-full overflow-y-auto animate-fade-in relative">
                ...
                <div className="flex justify-between items-center mb-6 sticky top-0 z-10 py-4 bg-[#0f0f23] -mt-6 -mx-6 px-6">
                    <Button onClick={resetToHome} className="bg-[rgba(15,10,30,0.6)] text-white hover:bg-[rgba(139,92,246,0.35)] !p-0 w-12 h-12 flex items-center justify-center">
                        <HomeIcon className="w-6 h-6" />
                    </Button>
                    <form onSubmit={handleSearch} className="flex-grow ml-4">
                        <div className="relative">
                            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search again..." className="w-full p-3 pl-12 rounded-lg bg-[rgba(15,10,30,0.6)] border border-[rgba(139,92,246,0.35)]" />
                            <SearchIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 text-[#e9d5ff]" />
                        </div>
                    </form>
                </div>
                    ...
                    {searchResults.length > 0 ? searchResults.map(source => (
                        <Card key={source.id} onClick={() => setSelectedSource(source)} className="p-0 overflow-hidden cursor-pointer group">
                            ...
                            <div className="p-4">
                                ...
                                <p className="text-sm text-[#e9d5ff]">{source.quotes.length} quotes</p>
                            </div>
                        </Card>
                    )) : (
                        <div className="col-span-full text-center py-16">
                            <h3 className="text-2xl font-bold">No Results Found</h3>
                            <p className="text-[#e9d5ff]">Try searching for something else, or create a new source.</p>
                        </div>
                    )}
                </div>
            </div>
        ...
        <>
            ...
            <div className="w-full h-full flex flex-col items-center justify-center p-4 quote-bg relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-[#e9d5ff] mb-8 max-w-lg">Search for quotes from your favorite sources, or create your own collection.</p>
                            ...
                            <form onSubmit={handleSearch} className="w-full">
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full p-5 pl-14 rounded-full bg-[#0f0f23]/50 border-2 border-[rgba(139,92,246,0.35)] backdrop-blur-sm text-lg focus:outline-none focus:ring-2 focus:ring-accent-primary text-black"
                                    />
                                    <SearchIcon className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-[#e9d5ff]" />
                                </div>
                            </form>
                            {showSuggestions && suggestions.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-[rgba(15,10,30,0.75)] rounded-xl shadow-2xl border border-[rgba(139,92,246,0.35)] overflow-hidden z-50 animate-fade-in max-h-96 overflow-y-auto">
                                    {suggestions.map((result, index) => (
                                        <div
                                            key={index}
                                            onClick={() => handleSuggestionClick(result)}
                                            className="p-3 hover:bg-[rgba(15,10,30,0.6)] cursor-pointer border-b border-[rgba(139,92,246,0.35)] last:border-0 transition-colors flex items-start gap-3"
                                        >
                                            ...
                                            <div className="flex-grow min-w-0">
                                                ...
                                                <p className="text-xs text-[#e9d5ff] mb-1">{result.source.type}</p>

                                                {result.match_type === 'quote' && result.matched_quotes.length > 0 && (
                                                    <p className="text-sm text-[#e9d5ff] line-clamp-2 italic">
